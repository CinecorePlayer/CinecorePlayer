...
namespace CinecorePlayer2025.Engines
{
    ...
    public sealed class DirectShowUnifiedEngine : IPlaybackEngine
    {
        ...
        private string? _lastYouTubeVideoId;
        ...
        public void Open(string mediaPath, bool hasVideo)
        {
            ...
            if (IsYouTubeUrl(mediaPath))
            {
                ...
                _lastYouTubeVideoId = yt.VideoId;
        ...
        private static bool TryResolveYouTubeStreams(string youtubeUrl, int? preferredHeight, out YouTubeResolveResult result, out string error)
        {
            ...
            {
                error = "yt-dlp non trovato. Metti 'yt-dlp.exe' nella cartella dell'app (o nel PATH).";
                return false;
            }
            ...
            try
            {
                ...
                {
                    error = "Nessun formato disponibile (formats mancante).";
                    return false;
                }
                ...
                {
                    error = "Nessun formato video trovato.";
                    return false;
                }
                ...
                {
                    error = "Impossibile selezionare un formato video.";
                    return false;
                }
                ...
                if (string.Equals(bestVideo.ACodec, "none", StringComparison.OrdinalIgnoreCase))
                {
                    if (audioCands.Count == 0)
                    {
                        ...
                        error = "Formato video selezionato senza audio e nessun formato audio trovato.";
            ...
            {
                error = "Errore parse yt-dlp JSON: " + ex.Message;
                return false;
            }
        }
        ...
        private static string? FindYtDlpExecutable()
        {
            try
            {
                // 1) accanto all'eseguibile (consigliato)
                ...
                var candidates = new[]
                {
                    ...
                    Path.Combine(baseDir, "bin", "yt-dlp")
                };
        ...
        private static bool TryGetYouTubeJson(string ytDlpExe, string youtubeUrl, string cacheKey, out string json, out string error)
        {
            ...
            try
            {
                ...
                {
                    FileName = ytDlpExe,
                    Arguments = "-J --no-playlist --no-warnings -- " + QuoteArg(youtubeUrl),
                ...
                if (!p.Start())
                {
                    error = "Impossibile avviare yt-dlp.";
                    return false;
                }
                ...
                {
                    error = "yt-dlp ha restituito errore (exit=" + p.ExitCode + "). " + (string.IsNullOrWhiteSpace(stderr) ? "" : stderr.Trim());
                    return false;
                }
            ...
            {
                error = "Errore yt-dlp: " + ex.Message + (string.IsNullOrWhiteSpace(stderr) ? "" : " | " + stderr.Trim());
                return false;
            }
        }
        ...
        private static string QuoteArg(string s)
        {
            ...
            // Se contiene spazi o caratteri speciali, quotiamo.
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (looksSub && name != null &&
                        ...
                        name.IndexOf("forced", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        // Preferenza: “Auto Forced” (madVR lo interpreta come no-subs)
                        sel.Enable(i, AMStreamSelectEnableFlags.Enable);
                        if (mt != null) DsUtils.FreeAMMediaType(mt);
                        if (a != null && Marshal.IsComObject(a)) Marshal.ReleaseComObject(a);
                        if (b != null && Marshal.IsComObject(b)) Marshal.ReleaseComObject(b);
                        _updateCb?.Invoke();
                        Dbg.Log("Subs disabilitati via Auto Forced");
                        return true;
                    }
                    if (looksSub && name != null &&
        ...
        private void DisposeGraph()
        {
            ...
            try { _basicAudio?.put_Volume(-10000); } catch { }
        ...
        public Bitmap? GetPreviewFrame(double seconds, int maxW = 360)
        {
            try
            {
                ...
                foreach (var off in offsets)
                {
                    ...
                    bmp = _thumb.Get(s2, maxW, realtime: true);
                    if (bmp != null) return bmp;
                }
            }
            catch (Exception ex)
